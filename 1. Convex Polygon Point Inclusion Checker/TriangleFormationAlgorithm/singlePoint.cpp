...
bool search(int i, int j, cv::Mat &image) {
    while (j != i + 1) {
        int m = std::floor((i + j) / 2);

        Point2d rearrangedIVertex = rearrangePoint(vertices[i].x, vertices[i].y);
        Point2d rearrangedJVertex = rearrangePoint(vertices[j].x, vertices[j].y);
        Point2d rearrangedMVertex = rearrangePoint(vertices[m].x, vertices[m].y);

        // Draw lines (i, m) and (m, j)
        cv::line(image, rearrangedIVertex, rearrangedMVertex, cv::Scalar(0, 0, 255), 3);
        cv::line(image, rearrangedMVertex, rearrangedJVertex, cv::Scalar(0, 0, 255), 3);
        cv::line(image, rearrangedIVertex, rearrangedJVertex, cv::Scalar(0, 0, 255), 3);
        cv::imshow("Convex Polygon Point Inclusion Checker", image);
        cv::waitKey(5000);

        // Redraw lines (i, m) and (m, j) in white to "erase" them
        cv::line(image, rearrangedIVertex, rearrangedMVertex, cv::Scalar(255, 255, 255), 3);
        cv::line(image, rearrangedMVertex, rearrangedJVertex, cv::Scalar(255, 255, 255), 3);
        cv::line(image, rearrangedIVertex, rearrangedJVertex, cv::Scalar(255, 255, 255), 3);
        for (size_t k = 0; k < vertices.size(); ++k) {
            std::vector <Point> cvPoints2;
            cvPoints2.reserve(vertices.size());
            for (const auto &p: vertices) {
                Point2d rearrangedVertex = rearrangePoint(p.x, p.y);
                cvPoints2.emplace_back(static_cast<int>(rearrangedVertex.x), static_cast<int>(rearrangedVertex.y));
            }

            cv::polylines(image, cvPoints2, true, cv::Scalar(255, 153, 51), 2, cv::LINE_AA);
            Point2d rearrangedKVertex = rearrangePoint(vertices[k].x, vertices[k].y);
            cv::circle(image, rearrangedKVertex, 9, cv::Scalar(255, 153, 51), -1);
        }
...
int main() {
    ...
    // Description
    cv::circle(image, Point(400, 1190), 9, cv::Scalar(0, 200, 0), -1);
    cv::putText(image, "Inside", Point(420, 1200), cv::FONT_HERSHEY_DUPLEX, 1.1, cv::Scalar(0, 200, 0), 2,
                cv::LINE_AA);

    cv::circle(image, Point(600, 1190), 9, cv::Scalar(0, 0, 255), -1);
    cv::putText(image, "Outside", Point(620, 1200), cv::FONT_HERSHEY_DUPLEX, 1.1, cv::Scalar(0, 0, 255), 2,
                cv::LINE_AA);
    ...
    cv::circle(image, rearrangePoint(randomPoint.x, randomPoint.y), 9, cv::Scalar(17, 190, 233), -1);

    bool isInside = search(0, 29, image);

    cv::Scalar qColor = isInside ? cv::Scalar(0, 200, 0) : cv::Scalar(0, 0, 255);
    cv::circle(image, rearrangePoint(randomPoint.x, randomPoint.y), 9, qColor, -1);
